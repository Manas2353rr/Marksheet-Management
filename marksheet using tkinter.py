...
def calClick():
    ...
    #Total Marks
    tot=(p.get() + c.get() + m.get())

    #Percentage
    pr=((p.get() + c.get() + m.get())/3)
...
def saveClick():
    ...
    data="{},{},{},{},{},{},{}\n".format(name.get(),p.get(),c.get(),m.get(),total.get(),percent.get(),grade.get())
    print('Data is',data)
    f=open('marksheet.csv','a')
    f.write(data)
    f.close()

    messagebox.showinfo('Saved','List Saved Successfully in File')
...
t3=Entry(win,width=20,state='readonly',textvariable=total,**dict3)
t3.grid(row=9,column=1)

l3=Label(win,text='Percent',**dict)
l3.grid(row=9,column=3)

t3=Entry(win,width=20,state='readonly',textvariable=percent,**dict3)
t3.grid(row=9,column=4)

l3=Label(win,text='Grade',**dict)
l3.grid(row=9,column=7,pady=30)

t3=Entry(win,width=20,state='readonly',textvariable=grade,**dict3)
...
b2=Button(win,width=20,text='Save',command=saveClick)
b2.grid(row=11,column=6,pady=30)

b3=Button(win,width=20,text='Plot',command=plotClick)
b3.grid(row=12,column=2)

b4=Button(win,width=20,text='Clear',command=clearClick)
b4.grid(row=12,column=6)

#Step I: We need to create menubar
mbr=Menu(win)
#Step II: Display Menubar in Tkinter window
win.config(menu=mbr)

#Step III: Creating Menus inside Menubar (Like File, Edit)
f=Menu(mbr, tearoff=0)  #Creating child menu of mbr
#Step IV: Creating MenuItems (add_command) inside menu
f.add_command(label='Calculate' , command=calClick)
f.add_command(label='Save' , command=saveClick)
f.add_command(label='Plot' , command=plotClick)
f.add_command(label='Clear' , command=clearClick)

#Step V: Add Menu into Menubar
mbr.add_cascade(label='File',menu=f)
